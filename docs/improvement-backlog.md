...
| 3.5 | Who is playing, saved progress and the Faith Journal | **Mostly done** | A "Who is playing?" screen (up to 4 children per tablet, each with a name and one of six paper pictures), and a Faith Journal per child holding the verses and charms they have earned, each read aloud when tapped. Saved on the tablet only (`profiles.cfg`). A press-and-hold "For grown-ups" area can empty a journal or remove a child. See `docs/faith-journal.md`. **Open:** drawing/colouring (four ideas sketched, none built), and a check on a real tablet, especially the on-screen keyboard for typing a name. |
...
| 5.6 | Tests | **Partly done** | One headless smoke test (154 checks: story, input devices, touch, pause, the play area, the lamb, fish, butterflies, where trees and rocks stand, the performance budget, profiles and the Faith Journal, voice-over, the soundscape and Steady Hands) now runs on every pull request and push to `main` through GitHub Actions (`.github/workflows/smoke-test.yml`, Godot 4.7.2 on Linux). It has run on GitHub and passes (about 35 seconds). Still open: nothing verifies how the game looks, so keep the screenshot helper in mind for visual checks. |
