...
The chapter's story is not finished. The place is built: the valley's ridge top is only a few steps deep, so the camp stands on its own wide clearing that carries on from it, framed by cypress and olive on soft rises, with chapter 1 laid out below a lookout stone. The king's round tent and three smaller tents face a stone-ring fire with breathing paper flames; one lantern hangs at the king's tent; four flags lean with the wind; four guards with staffs walk slow loops; an owl glides in to a cypress by the lookout and hoots while perched; fireflies drift at the edges; crickets and the fire's crackle replace the valley's birds. It is its own scene (`scenes/chapters/kings_camp.tscn`), loaded when the child goes there, so chapter 1 never pays for it. From the Faith Journey, The King's Camp opens that ground and turns the light to the blue hour, and chapter 1 stands down so none of its lines or keys can land on top of the camp. Jonathan stands by the fire. He is not a recolour of David: longer straight hair to his shoulders, a thin gold band in it, a longer face, wider eyes, a wine-red tunic and a gold sash. No crown. The look to judge is [jonathan-look.jpg](jonathan-look.jpg). His words, the three gifts and the friendship cord are in the camp now. She finds the robe, the bow and the belt, loops a cord three times, and receives 1 Samuel 18:1 and the Friendship charm. It ends the way chapter 1 does: the charm floats onto the Virtue Bracelet, then the cheer, confetti, "Chapter 2 Complete!" and the end card ("You earned the Friendship charm!", Play again, which replays the camp, Faith Journey, My journal, Colour my charm for Friendship, Keep exploring). The three words Knit, Loved and Friend light up one by one (`word_chip.gd`, shared with chapter 1's words): the next word breathes, a tapped word pops with a ring and sparkles and stays lit with a tick, and the words and the cord card sit above the dialogue bar so the line being read is never covered. The gift list is in pictures (the blue robe, the bow, the belt with its gold square), each ticked with a green tick as it is found (`gift_checklist.gd`), and after 14 seconds with nothing found chapter 1's golden arrow points at the nearest gift still in the camp (`wonder_item_hints.gd`). The cord card is short and wide, so it stays below the child in the camp view. With Easy words on, nine camp lines swap for simpler ones (`easy_text` in `assets/dialogue/kings_camp.tres`). The voice is Higgsfield now. Wonder Light is still Juno, the same narrator as chapter 1. Jonathan is Dylan, the user-selected male replacement for Julian; David remains Bram and Wonder Light remains Juno.
...
The clearing is the king's tent, three smaller tents, four banners, the fire and the lookout. The crates and extra tents are gone so the gifts and the valley read at a glance. People keep their paper colours in the blue hour; the fire warms a cheek and a pool of light on the tent wall. The moon is a crescent with a few paper stars. Found gifts sit beside David. Before the cord she taps Knit, Loved, and Friend. Each loop takes about four seconds, and the loops themselves sit in the firelight. She cannot walk during the arrival line. At the lookout stone, Wonder Light names the valley below, the stream comes back faintly, and one far sheep calls. Knit, Loved, and Friend are cut from Wonder Light's own recordings, so a tap uses her voice. The line on that screen is her recording of "Friends stay tied together." The lookout line ("Look. David's valley is still down there.") is now recorded in Wonder Light's voice too (`jn_lookout.wav`).
