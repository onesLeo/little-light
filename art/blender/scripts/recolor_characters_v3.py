"""
Little Light — recolor pass v3 (bright Godot-readable albedo).
- wonder_walker_v3.glb → wonder_walker_v5.glb
- david_mentor_v2.glb → david_mentor_v4.glb
- wonder_items_v2.glb → wonder_items_v4.glb
Grain mix <= 0.15; lifted grain floor so dark brown never dominates.
"""
import bpy
import os

WW_IN = os.environ.get("LL_WW_IN", "/workspace/little-light/little-light-godot/assets/wonder_walker_v3.glb")
WW_OUT = os.environ.get("LL_WW_OUT", "/workspace/little-light/little-light-godot/assets/wonder_walker_v5.glb")
DAVID_IN = os.environ.get("LL_DAVID_IN", "/workspace/little-light/little-light-godot/assets/david_mentor_v2.glb")
DAVID_OUT = os.environ.get("LL_DAVID_OUT", "/workspace/little-light/little-light-godot/assets/david_mentor_v4.glb")
ITEMS_IN = os.environ.get("LL_ITEMS_IN", "/workspace/little-light/little-light-godot/assets/wonder_items_v2.glb")
ITEMS_OUT = os.environ.get("LL_ITEMS_OUT", "/workspace/little-light/little-light-godot/assets/wonder_items_v4.glb")
...
WW_COLORS = {
    "Skin": (0.92, 0.72, 0.55, 1.0),
    "Tunic": (0.35, 0.55, 0.85, 1.0),   # soft SKY BLUE — must pop
    "Sash": (0.95, 0.35, 0.35, 1.0),    # CORAL
    "Hair": (0.18, 0.10, 0.07, 1.0),
    "Shoe": (0.38, 0.24, 0.16, 1.0),
    "Eye": (0.08, 0.08, 0.10, 1.0),
    "OL": (0.05, 0.04, 0.04, 1.0),
}

DAVID_COLORS = {
    "D_Skin": (0.90, 0.70, 0.52, 1.0),
    "D_Tunic": (0.90, 0.65, 0.25, 1.0),  # golden ochre
    "D_Sash": (0.25, 0.55, 0.35, 1.0),   # green
    "D_Hair": (0.16, 0.09, 0.06, 1.0),
...
ITEM_COLORS = {
    "I_Wool": (0.95, 0.92, 0.85, 1.0),     # bright cream
...
def tint_material(mat, color, mix_fac=GRAIN_MIX):
    ...
    for name, color in color_map.items():
        ...
        if mat:
            ...
            for m in bpy.data.materials:
                if name.lower() == m.name.lower() or name.lower() in m.name.lower():
                    mix = 0.0 if "ol" in m.name.lower() or "eye" in m.name.lower() or "hair" in m.name.lower() else GRAIN_MIX
                    tint_material(m, color, mix_fac=mix)
                    print("RECOLOR_FUZZY", m.name, "<-", name, color)
                    found = True
                    break
            if not found:
...
def main():
    ...
    print("DONE recolor_characters_v3")
